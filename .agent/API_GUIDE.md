...
2. Đơn vị giám sát chấp nhận yêu cầu trước khi yêu cầu nhận báo giá.
3. Khi chọn báo giá, công việc kế thừa đơn vị đã chấp nhận và bắt đầu ở `DangGiamSat`.
...
Cập nhật yêu cầu chưa chốt. Đổi `giamSatId` sẽ gửi lại lời mời bằng cách reset `trangThaiGiamSat` về `ChoDuyet`.
...
| `PUT` | `/jobs/:id/supervisor/accept` | Giám sát chấp nhận yêu cầu |
| `PUT` | `/jobs/:id/supervisor/reject` | Giám sát từ chối yêu cầu; body `{ "lyDo": "..." }` |
...
Chỉ gửi được sau khi `job.trangThaiGiamSat = "DaChapNhan"`.
...
Giám sát đã được phê duyệt ở mức yêu cầu trước khi tạo công việc. Công việc mới được tạo với `trangThaiGiamSat = "DangGiamSat"`; không còn endpoint chọn, chấp nhận hoặc từ chối giám sát ở mức contract.
...
```json
{
  ...
  "ngayGiamSatChapNhan": "2026-04-26T02:00:00.000Z",
...
- Yêu cầu mới luôn có `trangThai = DangNhanHoSo`, `yeuCauGiamSat = true`, `trangThaiGiamSat = ChoDuyet`.
...
Quy tắc trạng thái: `DangNhanHoSo -> DaDong | DaHuy`, `DaDong -> DaHuy`. Trạng thái `DaChot` chỉ phát sinh từ việc chấp nhận báo giá. Có thể đổi giám sát khi yêu cầu chưa chốt; việc đổi giám sát reset `trangThaiGiamSat` về `ChoDuyet`.

#### `PUT /jobs/:id/supervisor/accept`

Request: không có body.

Response: `{ "message": "Chap nhan giam sat yeu cau thanh cong", "job": JobWithDetailsObject }`, trong đó `job.trangThaiGiamSat = "DaChapNhan"`.

#### `PUT /jobs/:id/supervisor/reject`

Request:

```json
{
  "lyDo": "Khong du nguon luc vao thoi diem nay."
}
```

Response: `{ "message": "Tu choi giam sat yeu cau thanh cong", "job": JobWithDetailsObject }`, trong đó `job.trangThaiGiamSat = "TuChoi"` và `job.lyDoTuChoiGiamSat` chứa lý do.
...
Điều kiện: yêu cầu phải đang `DangNhanHoSo` và có `trangThaiGiamSat = DaChapNhan`; giá và thời gian phải lớn hơn `0`; một tài khoản freelancer không gửi hai báo giá cho cùng yêu cầu.
...
2. Xác nhận đơn vị giám sát đã duyệt yêu cầu (`TrangThaiGiamSat = DaChapNhan`).
3. Đổi báo giá đã chọn sang `DuocChon` và các báo giá khác sang `TuChoi`.
4. Đổi yêu cầu thành `DaChot`.
5. Tạo công việc kế thừa giám sát với `TrangThaiGiamSat = DangGiamSat`.
...
### Refund Requests - Thoa Thuan Hoan Tien Truoc Tranh Chap

Luong nghiep vu:

```text
Nguoi thue -> POST /refund-requests
            -> Freelancer PUT /refund-requests/:id/accept
               -> hoan tien, ket thuc hop dong
            -> hoac PUT /refund-requests/:id/reject
               -> tu dong tao TranhChap cho don vi giam sat phan xu
```

Yeu cau co the gui khi escrow cua hop dong van dang duoc he thong giu. Khong tao them yeu cau moi neu da co yeu cau dang cho freelancer, hop dong da huy/dang tranh chap, hoac escrow da duoc phan bo.
...
```json
{
  "congViecId": 3,
  "nguoiThueId": 1,
  "lyDo": "Muon dung du an va hoan lai tien escrow",
  "moTa": "Hai ben co the ket thuc som neu freelancer dong y."
}
```

Response tra ve `refundRequest` voi `trangThai: "ChoFreelancerDuyet"` va cac khoan du kien. Vi du `GiaThoa = 9500000`, `TongEscrow = 9950000`:

```json
{
  "phiHeThong": "475000",
  "tienFreelancer": "950000",
  "tienGiamSat": "950000",
  "tienHoan": "7575000"
}
```

Phep tinh khi dong y: `phiHeThong = 5% * GiaThoa`, `tienFreelancer = 10% * GiaThoa`, `tienGiamSat = 10% * GiaThoa`, `tienHoan = TongEscrow - ba khoan tren`.
...
Backend danh dau escrow `DaHoan`, tao cac giao dich `HoanTien`, `ThanhToanCuoi`, `PhiGiamSat`, `PhiHeThong`, va chuyen cong viec sang `DaHuy`.
...
Backend chuyen cong viec dang thuc hien sang `TranhChap`, tao `TranhChap` `MoiMo` cho don vi giam sat phan xu. Tien escrow chua bi phan bo.

#### `GET /refund-requests/:id` va `GET /contracts/:id/refund-requests`

Hai API tra trang thai yeu cau, bang tinh chia tien va `tranhChapId` neu freelancer da tu choi.
